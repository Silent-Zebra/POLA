"""
Adapted from https://github.com/011235813/lio/blob/master/lio/env/room_symmetric_baseline.py
...
class EscapeRoom(gym.Env):
    ...
    def __init__(self, max_steps, n_agents=2, reward_value=2.0,
        ...
        self.n_actions = 3 * 2 ** (
                    self.n_agents - 1) if incentivization_inside_env else 3
        ...
        self.action_space = Tuple(
            [Discrete(self.n_actions) for _ in range(self.n_agents)])
        self.observation_space = Tuple(
            [OneHot(self.l_obs) for _ in range(self.n_agents)])
        ...
        if not self.solved:
            for agent_id in range(0, self.n_agents):
                ...
                if self.incentivization_inside_env:
                    ...
                    rewards[agent_id] -= (
                                self.map_action_to_give[actions[agent_id]] *
                                self.reward_coeff * self.reward_value)
                    # Received rewards
                    if self.n_agents == 2:
                        other_id = 1 - agent_id  # only for the case of 2 agents
                        r = self.map_action_to_give[
                                actions[agent_id]] * self.reward_value
                        rewards[other_id] += r
                        given[0] += r
                    elif self.n_agents == 3:
